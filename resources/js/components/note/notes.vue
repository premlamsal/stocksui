      ...
      <b-modal id="bv-modal-add-note" hide-footer>
        ...
        <div class="d-block">
          <div class="form-group">
            ...
            <span v-if="errors.title" :class="['errorText']">{{ errors.title[0] }}</span>
          </div>
       
          <div class="form-group">
            <label for="Description">Description:</label>
            <textarea v-model="note.description" :class="['form-control']"></textarea>
    ...
    methods: {
          ...
          if (result.value) {
            axios.delete('/api/note/' + id)
              .then(function(response) {
                ...
                let index_to_delete = currObj.notes.findIndex(note => note.id === id)
      ...
      autoCompleteTable() {
        ...
        if (this.searchTableKey != '') {
          ...
          let currObj = this;
          axios.post('/api/notes/search', { searchQuery: this.searchTableKey })
            .then(function(response) {
